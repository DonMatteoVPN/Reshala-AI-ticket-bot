# Решала Support от DonMatteo

Telegram бот для технической поддержки с AI-автоответами и Mini App для менеджеров.

## 🚀 Возможности

- 🤖 **AI-автоответы** — поддержка 5 провайдеров с автоматическим переключением при ошибках
- 📱 **Telegram Mini App** — панель управления для менеджеров
- 💬 **Система тикетов** — статусы: 💬 открыт → 🔥 эскалация → 🚨 подозрительный → ✅ закрыт
- 📚 **База знаний** — контекст для AI из редактируемых статей
- 🔍 **Поиск пользователей** — интеграция с Remnawave API
- 💰 **Баланс** — интеграция с Bedolaga API

---

## 📦 Установка через Docker Compose

### Шаг 1: Клонирование репозитория

```bash
git clone https://github.com/your-repo/reshala-support.git
cd reshala-support
```

### Шаг 2: Создание файла `.env`

Создайте файл `.env` в корне проекта:

```bash
cp .env.example .env
nano .env
```

Заполните переменные:

```env
# ═══════════════════════════════════════════════════════════
#                    ОБЯЗАТЕЛЬНЫЕ НАСТРОЙКИ
# ═══════════════════════════════════════════════════════════

# Telegram Bot — получить у @BotFather
BOT_TOKEN=1234567890:ABCDEFghijklmnop_qrstuvwxyz123456789

# Remnawave Panel API
REMNAWAVE_API_URL=https://your-panel.example.com
REMNAWAVE_API_TOKEN=your_jwt_token_from_panel

# Группа поддержки — ID группы с топиками (начинается с -100)
SUPPORT_GROUP_ID=-1001234567890

# ID менеджеров через запятую (получить у @userinfobot)
ALLOWED_MANAGER_IDS=123456789,987654321

# ═══════════════════════════════════════════════════════════
#                    URL ДЛЯ MINI APP
# ═══════════════════════════════════════════════════════════

# Публичный URL бэкенда для Mini App
# Для локальной разработки: http://localhost:8001
# Для продакшена: https://api.your-domain.com
REACT_APP_BACKEND_URL=https://api.your-domain.com

# ═══════════════════════════════════════════════════════════
#                    ОПЦИОНАЛЬНО (Bedolaga)
# ═══════════════════════════════════════════════════════════

# Bedolaga API для проверки баланса (если используется)
# BEDOLAGA_API_URL=https://bedolaga.example.com
# BEDOLAGA_API_TOKEN=your_bedolaga_token
```

### Шаг 3: Запуск

```bash
docker compose up -d
```

Проверка статуса:

```bash
docker compose ps
```

Вы должны увидеть 3 работающих контейнера:
- `reshala-mongodb` — база данных
- `reshala-backend` — API и бот
- `reshala-frontend` — Mini App

### Шаг 4: Настройка Mini App в BotFather

1. Откройте @BotFather в Telegram
2. Отправьте `/mybots` → выберите вашего бота
3. **Bot Settings** → **Menu Button** → **Configure Menu Button**
4. Введите URL вашего Mini App: `https://your-domain.com`
5. Введите название кнопки: `Панель`

---

## 🔧 Порты и URL

| Сервис | Внутренний порт | Внешний порт | URL |
|--------|-----------------|--------------|-----|
| Backend API | 8001 | 8001 | http://localhost:8001 |
| Frontend | 80 | 3000 | http://localhost:3000 |
| MongoDB | 27017 | — | Только внутренний |

---

## 📊 Просмотр логов

```bash
# Все сервисы
docker compose logs -f

# Только backend (бот + API)
docker compose logs -f backend

# Только frontend
docker compose logs -f frontend

# Последние 100 строк backend
docker compose logs --tail=100 backend
```

---

## 🔄 Обновление

```bash
# Остановка
docker compose down

# Получение обновлений
git pull

# Пересборка и запуск
docker compose up -d --build
```

---

## 🛠 Troubleshooting

### Бот не запускается

1. Проверьте `BOT_TOKEN`:
```bash
docker compose logs backend | grep -i "bot"
```

2. Убедитесь что токен правильный (должен быть вида `123456789:ABCdef...`)

### Mini App показывает "Доступ запрещён"

1. Проверьте что ваш Telegram ID добавлен в `ALLOWED_MANAGER_IDS`
2. Узнать свой ID: отправьте любое сообщение боту @userinfobot

### Не работает поиск пользователей

1. Проверьте `REMNAWAVE_API_URL` и `REMNAWAVE_API_TOKEN`
2. Убедитесь что API панели доступен с сервера

### MongoDB не запускается

```bash
# Проверка healthcheck
docker compose ps

# Если mongodb unhealthy:
docker compose restart mongodb
```

### Очистка и полный перезапуск

```bash
docker compose down -v  # Удалит все данные!
docker compose up -d --build
```

---

## 🔐 Ротация логов

Docker автоматически ротирует логи:
- **Backend**: до 50MB × 5 файлов = max 250MB
- **Frontend**: до 10MB × 3 файла = max 30MB
- **MongoDB**: до 10MB × 3 файла = max 30MB

---

## 📁 Структура проекта

```
reshala-support/
├── .env                    # Переменные окружения
├── docker-compose.yml      # Docker конфигурация
├── backend/
│   ├── Dockerfile
│   ├── server.py           # FastAPI точка входа
│   ├── database.py         # MongoDB подключение
│   ├── bot/                # Telegram бот
│   │   ├── main.py
│   │   └── handlers/
│   ├── routers/            # API эндпоинты
│   └── services/           # Бизнес-логика
│       └── ai/
│           └── manager.py  # AI провайдеры с failover
├── frontend/
│   ├── Dockerfile
│   ├── nginx.conf
│   └── src/
│       ├── App.js
│       ├── pages/
│       └── components/
└── memory/
    ├── PRD.md
    └── CHANGELOG.md
```

---

## 🤖 AI Провайдеры

Настройка через Mini App → AI Провайдеры:

| Провайдер | Модели | Получить ключ |
|-----------|--------|---------------|
| **Groq** | llama-3.1, mixtral | [console.groq.com](https://console.groq.com) |
| **OpenAI** | GPT-4o, GPT-4 | [platform.openai.com](https://platform.openai.com) |
| **Anthropic** | Claude 3 | [console.anthropic.com](https://console.anthropic.com) |
| **Google** | Gemini Pro, Flash | [aistudio.google.com](https://aistudio.google.com) |
| **OpenRouter** | Все модели | [openrouter.ai](https://openrouter.ai) |

**Автоматический failover**: если один ключ перестаёт работать, система автоматически переключается на следующий.

---

## 📞 API Эндпоинты

### Настройки
- `GET /api/settings` — получить настройки
- `PUT /api/settings` — обновить настройки

### AI
- `POST /api/ai/chat` — отправить сообщение AI
- `PUT /api/ai/provider/{name}` — обновить провайдера
- `POST /api/ai/provider/{name}/test` — тестировать ключ

### Тикеты
- `GET /api/tickets/active` — активные тикеты
- `POST /api/tickets/{id}/reply` — ответить на тикет
- `POST /api/tickets/{id}/close` — закрыть тикет

### Поиск
- `POST /api/lookup` — поиск пользователя по ID/username

### База знаний
- `GET /api/knowledge/articles` — список статей
- `POST /api/knowledge/articles` — создать статью
- `PUT /api/knowledge/articles/{id}` — обновить
- `DELETE /api/knowledge/articles/{id}` — удалить

---

## 📄 Лицензия

MIT License

---

## 💬 Поддержка

- Telegram: @DonMatteo
- GitHub Issues
